...
module.exports = {
  ...
  async execute (message, args, commandName, client) {
    const mykiUser = message.author.id
    const sub = args[0]
    if (!sub) {
      const reply = new MessageEmbed()
        .setColor('RED')
        .setDescription('INVALID OPTION')
        .addField('register', 'Register your Myki account', true)
        .addField('balance', 'Check your account balance', true)
        .addField('delete', 'Delete your myki account', true)
      message.reply({ embeds: [reply] })
    }

    if (sub === 'register') {
      const mykiRegisterNumber = args[1]
      mykiCard.findOne({
        userId: mykiUser
      }, async (err, data) => {
        ...
        if (!data) {
          ...
          const mykiRegister = new MessageEmbed()
            ...
            .setDescription(`Thank you for registering your myki! ${message.author}`)
            .setTimestamp()

          message.reply({ embeds: [mykiRegister] })
        } else {
          const mykiAlrRegister = new MessageEmbed()
            .setColor('RED')
            .setTitle('Myki Already Registered!')
            .setDescription(`Your Myki is Already Registered! ${message.author}`)
            .setTimestamp()

          message.reply({ embeds: [mykiAlrRegister] })
        }
        data.save()
      })
    } else if (sub === 'balance') {
      mykiCard.findOne({
        ...
        if (data) {
          ...
          if (mykiData.error) {
            return message.reply({ embeds: [new MessageEmbed().setTitle('ERROR').setDescription('Your Myki number is invalid!')] })
          }
          ...
          const mykiBalanceEmbed = new MessageEmbed()
            .setColor('#C2D840')
            .setTitle(`${message.author.username}'s Myki Balance`)
            .setThumbnail(message.author.user.displayAvatarURL({ dynamic: true }))
            .addFields([{
              name: 'Balance',
          ...
          message.reply({ embeds: [mykiBalanceEmbed] })
        } else {
          message.reply({ embeds: [new MessageEmbed().setTitle('ERROR').setDescription('You Must Register your Myki First!')] })
        }
      })
    } else if (sub === 'delete') {
      mykiCard.findOne({
        ...
        if (data) {
          ...
          message.reply({ embeds: [mykiDelete] })
        }
      })
    }
  }
